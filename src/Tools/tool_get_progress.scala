  ...
  def theory_states_json(
    ...
    val theory_states = JSON.Object(theories.map { name =>
      ...
      session.origin(name) -> (JSON.Object(
        ...
        PIDE_MCP_Commands.prefix_command_status_keys(commands))
    }: _*)
    ...
    theories.flatMap { name =>
      ...
      PIDE_MCP_Commands.state_entries_theory_dynamic(node_snapshot, None).toList.flatMap { entry =>
        Option.when(PIDE_MCP_Commands.status(node_snapshot, entry.cmd).is_running) {
          val command_state = PIDE_MCP_Commands.state_entry_json(node_snapshot, entry, doc, opts)
...
class Tool_Get_Progress extends PIDE_MCP_Tool("get_progress") {
  ...
  def handle(params: JSON.Object.T): Exn.Result[JSON.T] = Exn.capture {
    ...
    JSON.Object(
      PIDE_MCP_Commands.command_status_key(PIDE_MCP_Commands.Status.running) -> commands_running) ++
      Tool_Get_Progress.theory_states_json(session, snapshot, theory_names)
  }
}
