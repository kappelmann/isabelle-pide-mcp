  ...
  val all: List[(String, ToolDef)] = List(
    ...
    "read_theory" -> ToolDef(
      ...
      inputSchema = Map("type" -> "object", "properties" -> Map(
        ...
        "end_line" -> Map("type" -> "number", "description" -> "End line (inclusive)")
      ), "required" -> List("path"))
    ),
    ...
    "edit_theory" -> ToolDef(
      ...
      inputSchema = Map("type" -> "object", "properties" -> Map(
        ...
        "start_line" -> Map("type" -> "number", "description" -> "First line to replace (1-indexed)"),
        "end_line" -> Map("type" -> "number", "description" -> "Last line to replace (inclusive, defaults to start_line)"),
        ...
        "timeout_secs" -> Map("type" -> "number", "description" -> "Timeout in seconds for PIDE processing (default: 15)")
    ...
    "get_state" -> ToolDef(
      ...
      inputSchema = Map("type" -> "object", "properties" -> Map(
        ...
        "timeout_secs" -> Map("type" -> "number", "description" -> "Timeout in seconds for loading theory (default: 15)")
      ), "required" -> List("path"))
    ),
    ...
    "find_definition" -> ToolDef(
      ...
      inputSchema = Map("type" -> "object", "properties" -> Map(
        ...
        "line" -> Map("type" -> "number", "description" -> "Line number of the usage"),
